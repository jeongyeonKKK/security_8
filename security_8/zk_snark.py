...
def trust_middle(cv, Xt, t_0):
    ...
    for i in range(0,len(A)):
        if int(A[i]) == 0:
            B.append(0)
            continue
        B.append(cv.mul_point(int(A[i])+cv.order,G))
    return B
...
def public_2_x(public_1_x, k_x):
    ...
    for i in range(len(public_1_x)):
        if type(public_1_x[i]) == int:
            A.append(0)
            continue
        A.append(cv.mul_point(k_x, public_1_x[i]))
    return A
...
public_2_a = public_2_x(public_1_a, k_a)
public_2_b = public_2_x(public_1_b, k_b)
public_2_c = public_2_x(public_1_c, k_c)
...
public_3 = public_2_x(trust_middle(cv, (At+Bt+Ct), t_0), b)
...
class verifier():
    ...
    def verify_1(self,AT, BT, CT, HT, p_n_a, p_n_b, p_n_c, p_n_h):
        ...
        # verifier
        self.v_n_a = cv.mul_point(int(A_t_0), G)
        self.v_n_b = cv.mul_point(int(B_t_0), G)
        self.v_n_c = cv.mul_point(int(C_t_0), G)
        self.v_n_h = cv.mul_point(int(H_t_0), G)
    ...
    def verify_2(self, p_n_a_2, p_n_b_2, p_n_c_2,):
        self.v_n_a_2 = cv.mul_point(k_a, self.v_n_a)
        self.v_n_b_2 = cv.mul_point(k_b, self.v_n_b)
        self.v_n_c_2 = cv.mul_point(k_c, self.v_n_c)

        return (self.v_n_a_2 == p_n_a_2 and self.v_n_b_2 == p_n_b_2 and self.v_n_c_2 == p_n_c_2)

    def verify_3(self, p_n):
        v_n = cv.mul_point(b, (self.v_n_a + self.v_n_b + self.v_n_c))
        return p_n == v_n
...
class prover():
    ...
    def proof_1_x(self, public_1_x):
        ...
        for i in range(len(self.s)):
            ...
            if chk == 0:
                chk = 1
                A = cv.mul_point(int(self.s[i]), public_1_x[i])
            else:
                A = A + cv.mul_point(int(self.s[i]), public_1_x[i])
        return A

    def get_proof_1(self):
